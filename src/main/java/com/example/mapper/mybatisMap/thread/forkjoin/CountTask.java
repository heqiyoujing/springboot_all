package com.example.mapper.mybatisMap.thread.forkjoin;
 ...
 * ForkJoinTask:我们要使用Fork/Join框架，首先需要创建一个ForkJoin任务。该类提供了在任务中执行fork和join的机制。
 * 通常情况下我们不需要直接集成ForkJoinTask类，只需要继承它的子类，Fork/Join框架提供了两个子类：
 * RecursiveTask:用于有返回结果的任务;RecursiveAction：用于没有返回结果的任务.
 * ForkJoinPool:ForkJoinTask(CountTask)需要通过ForkJoinPool来执行
 */
